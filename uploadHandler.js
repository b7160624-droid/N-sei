...
var videoSkipCommand = '-ss $(echo "$(ffprobe -show_entries format=duration -of';
videoSkipCommand += ' default=noprint_wrappers=1:nokey=1 ';
videoSkipCommand += '{$path})/(100/{$percentage})" | bc) ';
var videoThumbCommand = 'ffmpeg -i {$path} -y {$skip}-vframes 1 -vf scale=';
...
exports.loadSettings = function() {
  ...
  videoPercentage = settings.videoThumbPercentage || 0;
...
exports.generateVideoThumb = function(identifier, file, tooSmall, audio,
    callback) {
  ...
  var thumbCallback = function(error) {
  ...
  if (native) {
    ...
    if (tooSmall) {
      widthToUse = file.width;
      heightToUse = file.height;
    } else if (file.width > file.height) {
      widthToUse = thumbSize;
      heightToUse = Math.floor(file.height * (thumbSize / file.width));
    } else {
      heightToUse = thumbSize;
      widthToUse = Math.floor(file.width * (thumbSize / file.height));
    }

    return native.thumbNailVideo(file.pathInDisk, thumbDestination, widthToUse,
        heightToUse, thumbCallback, false, videoPercentage);
  }
  ...
  command = command.replace('{$skip}', (videoPercentage > 0
      && videoPercentage < 100 && !audio) ? videoSkipCommand.replace('{$path}',
      file.pathInDisk).replace('{$percentage}', videoPercentage) : '');
...
exports.generateImageThumb = function(identifier, file, callback) {
  ...
  if (file.mime !== 'image/gif' || !ffmpegGif) {
    ...
    if (native) {
      ...
      return native.thumbNailVideo(file.pathInDisk, thumbDestination,
          widthToUse, heightToUse, thumbCb, true, 0);
...
exports.generateThumb = function(identifier, file, callback) {
  ...
  var mediaCondition = file.mime.indexOf('video/') > -1;
  mediaCondition = mediaCondition || file.mime.indexOf('audio/') > -1
  ...
  } else if (mediaCondition && mediaThumb) {
    exports.generateVideoThumb(identifier, file, tooSmall, mediaCondition
        && file.mime.indexOf('audio/') > -1, callback);
  } else {
