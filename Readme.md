...
* `videoThumbPercentage`(Number): when making thumbnails for videos, skip to this percentage of the video. Ignored if below 1 or above 99.
...
* `bypassTorMaxPosts`(Number): how many uses a block bypass will have before expires for tor users.
* `bypassEarlyExpirationMinutes`(Number): how many minutes a bypass will last before being used for the first time. If none is set, bypasses will not expire any early if not used.
* `bypassDurationHours`(Number): how many hours a block bypass will last. Defaults to 24.
* `bypassTorDurationHours`(Number): how many hours a block bypass will last for tor users.
